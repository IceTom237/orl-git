...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(curSong == 'kill streak'){
			...
			timeBar.createFilledBar(0xFF000000, 0xFF0B4AC8);
			camHUD.visible = false;

			scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.WHITE);
			healthBar.visible = false;
			healthBarBG.visible = false;
		}

		if(curSong == 'unwanted pirate'){
		...
		if(curSong == 'foraging'){
			...
			timeBar.createFilledBar(0xFF000000, 0xFF122D75);
			camHUD.visible = false;
			...
			GameOverSubstate.characterName = 'pico-player';
		}

		if(curSong == 'torn apart'){
			Lib.application.window.title = "One Ring Left";
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) {
				...
				if(curSong == 'kill streak') opponentStrums.members[i].visible = false;
			}
	...
	override public function update(elapsed:Float)
	{
		...
		super.update(elapsed);
	 	...
	 	switch (SONG.song.toLowerCase()) //SONG INFO BARS
		{
			case 'kill streak':
				scoreTxt.text = 'Blueballed: ' + deathCounter;
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				else if(!daNote.mustPress && curSong == 'kill streak')
				{
					daNote.active = true;
					daNote.visible = false;
				}
		...
		if(curSong == 'perfection' && songMisses > 0)
		{
		...
		if(curSong == 'kill streak' && songMisses > 5)
		{
			boyfriend.stunned = true;
			deathCounter++;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (daRating == 'sick' && daRating == 'good' && daRating == 'bad' && daRating == 'shit' && curSong == 'perfection' && !note.noteSplashDisabled)
			spawnNoteSplashOnNote(note);
		...
		if(curSong == 'perfection')
		{
			pixelShitPart1 = 'perfection/';
			pixelShitPart2 = '-perfect';
		}
	...
	override function stepHit()
	{
		...
		if(klsIntro)
		{
			if (curStep == 1)
			{
				...
				FlxG.camera.flash(FlxColor.BLACK, 56.6);
			}
		...
		if(foraginglol)
		{
			if (curStep == 1)
			{
				FlxG.camera.flash(FlxColor.BLACK, 4.75);
			}
